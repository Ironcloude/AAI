...
def pad_sequences_custom(sequences, maxlen, padding='pre'):
    ...
    for i, seq in enumerate(sequences):
        seq_len = len(seq)
        if seq_len > maxlen:
            if padding == 'pre': padded[i] = seq[-maxlen:]
            else: padded[i] = seq[:maxlen]
        else:
            if padding == 'pre': padded[i, maxlen-seq_len:] = seq
            else: padded[i, :seq_len] = seq
    return padded
...
def load_and_prepare_sequences(filepath='insta_clean_data.csv', min_seq_length=3, max_lookback=20):
    ...
    user_sequences = df.groupby('user_id')['product_id_encoded'].apply(list).values
    user_sequences = [seq for seq in user_sequences if len(seq) >= min_seq_length]
...
def train_recommender(run_id="C1", output_dir="recommender_results"):
    ...
    X, y, vocab_size, label_encoder = load_and_prepare_sequences(max_lookback=run_config.max_lookback)
    if X is None: return
    ...
    model = RecommendationLSTM(vocab_size, run_config.embed_dim, run_config.hidden_dim).to(device)
    ...
    for epoch in range(run_config.epochs):
        ...
        print(f"Epoch {epoch+1}/{run_config.epochs} | Loss: {avg_loss:.4f} | Test Acc: {acc:.4f}")
    
    metrics.elapsed_time_min = (time.time() - start_time) / 60
    
    # Exports
    os.makedirs(output_dir, exist_ok=True)
    save_file(model.state_dict(), os.path.join(output_dir, f'{run_config.run}.safetensors'))
    np.save(os.path.join(output_dir, f'{run_config.run}_label_classes.npy'), label_encoder.classes_)
